...
public class LevelSelectorScreen extends AbstractScreen   {
	...
	int _state;
	int _pageNumber;
	int LevelWidth = Gdx.app.getGraphics().getWidth();
	int LevelHeight = Gdx.app.getGraphics().getHeight();
	
	
	public LevelSelectorScreen(AgeOfTower aot) {		
		super(aot);
		_aot = aot;
		super.backstate = STATE_LEVELSELECT;
		...
		//-- constant pratique
		_targetOffsetPos = LevelWidth/2-LevelHeight/2;
		_startTargetOffsetPos = LevelWidth/2-LevelHeight/2;
		_bg_offset = -_targetOffsetPos*FLINGING_BG_REDUCER;

		//-- image de fond
		_backGround = new Image( _lssSkin.get("background",TextureRegion.class));
		_backGround.setSize(_backGround.getWidth()*LevelHeight/_backGround.getHeight(), LevelHeight);
		...
		while(itr.hasNext()){
			...
			if(pageList.size()<page+1){
				...
				panel_bg.setSize(LevelHeight, LevelHeight);
				panel_bg.setPosition(0, 0);
				
				currWidgetPage.setPosition(LevelWidth*page, 0);
			...
			//-- bouton, verouillþýskin different: lvl*bw () black and white / lvl*
			if(currLvl.isLocked()){
				imgBut = new ImageButton(_lssSkin.get("lvl"+new Integer(i+1)+"bw", ImageButtonStyle.class));
				imgBut.setSize(LevelHeight/3/imgBut.getHeight()*imgBut.getWidth(),LevelHeight/3);
			...
			float buttonLeftOffset = (LevelHeight-((imgBut.getWidth())*3+LAYOUT_BUTTON_PADDING*2))/2;
			
			imgBut.setPosition(
					buttonLeftOffset+(imgBut.getWidth()+LAYOUT_BUTTON_PADDING)*colone, 
					LevelHeight/2+LAYOUT_BUTTON_PADDING/2-(LAYOUT_BUTTON_PADDING+imgBut.getHeight())*(ligne)
					);
			i++;
			colone = i%3;
			ligne = (i/3)%2;
	...
	public void mySetPosition(float x){
		if(x<_startTargetOffsetPos && x>-(_pageNumber-1)*LevelWidth+_startTargetOffsetPos){
